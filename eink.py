...
def four_quadrants():
    ...
    for event in events:
        ...
        item = {
            ...
            'left': '[{}][{}]'.format('{:02d}:{:02d}'.format(start.hour, start.minute), event['cal']),
            'left_red': True,
        ...
        if event['cal'] in ['Study', 'Work', 'NYU']:  # Important
            if start <= (NOW + datetime.timedelta(hours=12)):  # Urgent
    ...
    red_layer = Image.new('1', (EPD_WIDTH, EPD_HEIGHT - WEATHER_HEIGHT), 1)
    black_layer = Image.new('1', (EPD_WIDTH, EPD_HEIGHT - WEATHER_HEIGHT), 1)

    red_layer_draw = ImageDraw.Draw(red_layer)
    black_layer_draw = ImageDraw.Draw(black_layer)

    red_card_layer, black_card_layer = quadrant_card(urgent_important, int((EPD_WIDTH - CALENDAR_WIDTH) / 2), int((EPD_HEIGHT - WEATHER_HEIGHT) / 2))

    red_layer.paste(red_card_layer, (0, 0))
    black_layer.paste(black_card_layer, (0, 0))

    red_card_layer, black_card_layer = quadrant_card(not_urgent_important, int((EPD_WIDTH - CALENDAR_WIDTH) / 2), int((EPD_HEIGHT - WEATHER_HEIGHT) / 2))

    red_layer.paste(red_card_layer, (int(EPD_WIDTH / 2 + 2), 0))
    black_layer.paste(black_card_layer, (int(EPD_WIDTH / 2 + 2), 0))

    red_card_layer, black_card_layer = quadrant_card(urgent_not_important, int((EPD_WIDTH - CALENDAR_WIDTH) / 2), int((EPD_HEIGHT - WEATHER_HEIGHT) / 2))

    red_layer.paste(red_card_layer, (0, int((EPD_HEIGHT - WEATHER_HEIGHT) / 2 + 2)))
    black_layer.paste(black_card_layer, (0, int((EPD_HEIGHT - WEATHER_HEIGHT) / 2 + 2)))

    red_card_layer, black_card_layer = quadrant_card(not_urgent_not_important, int((EPD_WIDTH - CALENDAR_WIDTH) / 2), int((EPD_HEIGHT - WEATHER_HEIGHT) / 2))

    red_layer.paste(red_card_layer, (int(EPD_WIDTH / 2 + 2), int((EPD_HEIGHT - WEATHER_HEIGHT) / 2 + 2)))
    black_layer.paste(black_card_layer, (int(EPD_WIDTH / 2 + 2), int((EPD_HEIGHT - WEATHER_HEIGHT) / 2 + 2)))

    black_layer_draw.line((0, (EPD_HEIGHT - WEATHER_HEIGHT) / 2, EPD_WIDTH, (EPD_HEIGHT - WEATHER_HEIGHT) / 2), 0, 2)
    black_layer_draw.line((EPD_WIDTH / 2, 0, EPD_WIDTH / 2, EPD_HEIGHT - WEATHER_HEIGHT), 0, 2)

    debug(red_layer, black_layer)
...
def quadrant_card(items, width, height):
    ...
    if len(items) == 0:
        text = 'No more task or event for the next 3 days! Good job!'
